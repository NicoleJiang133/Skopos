...
const FINISH_HOOK = `<button id="sk-finish" style="position:fixed;top:10px;right:14px;z-index:9999;padding:8px 14px;background:#e8b04a;color:#1a1611;border:0;border-radius:6px;font:600 12px ui-monospace,monospace;letter-spacing:1px;cursor:pointer;box-shadow:0 2px 12px rgba(0,0,0,.4)">FINISH JOURNEY →</button><script>addEventListener('DOMContentLoaded',()=>{const end=()=>{try{window.SkoposReactor&&window.SkoposReactor.disconnect()}finally{location.assign('/#finish')}};document.getElementById('sk-finish').onclick=end;const b=document.getElementById('rx-end');if(b)b.onclick=end})</script></body>`
