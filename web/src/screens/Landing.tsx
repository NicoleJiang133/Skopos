...
import { scanVenue, type ScanResult } from '../backend/handoff'
...
export function Landing() {
  ...
  const setStoredScan = useStore((s) => s.setScan)
  ...
  const beginHold = () => {
    ...
    const tick = () => {
      ...
      if (p >= 1) {
        ...
        window.setTimeout(() => goto('play'), 1100)
        return
      }
    ...
    >
      ...
      {(phase === 'found' || phase === 'entering') &&
        objects.slice(0, revealed).map((o, i) => (
          <div key={o.id || i} style={{ position: 'absolute', ...tagPosition(i, objects.length) }}>
            <span
              style={{
                position: 'absolute',
                width: 14,
            ...
            >
              ...
              <span style={{ color: 'var(--sk-text-dim)' }}>
                {o.material ? ` · ${o.material}` : ''} · {Math.round(o.confidence * 100)}%
              </span>
            </div>
          </div>
        ))}
        ...
        <div style={{ maxWidth: 620 }}>
          ...
          {phase === 'idle' && (
            <>
              ...
              <p style={lede}>
                Upload a photo of your space, then play inside it — every table you move and route
                you pick teaches Skopos how you like your robots to work.
              </p>
              <button
                className="sk-panel"
              ...
              <div style={hint}>Step 1 — drop or upload a photo of your physical venue.</div>
            </>
          )}

          {phase === 'ready' && (
            <div style={{ animation: 'sk-rise 900ms var(--sk-ease) 1.4s both' }}>
              <h1 style={h1}>This is your venue. Let’s wake the robot inside it.</h1>
              <p style={lede}>
                We’ll scan the room for the things your robot will work around, then hand you its
                eyes.
              </p>
          ...
          {phase === 'scanning' && (
            <div style={{ animation: 'sk-rise 500ms var(--sk-ease) both' }}>
              ...
              <p style={lede}>Surfaces, furniture, hazards — everything the robot must respect.</p>
            </div>
          )}
          ...
          {(phase === 'found' || phase === 'entering') && (
            <div style={{ animation: 'sk-rise 500ms var(--sk-ease) both' }}>
              ...
              <p style={lede}>
                Hold to wake it. From here on you see, move and decide as the robot does.
              </p>
              <button
                ...
                style={{
                  ...
                  cursor: 'pointer',
                  animation: hold > 0 ? 'sk-heartbeat 900ms ease-in-out infinite' : undefined,
                  touchAction: 'none',
                  userSelect: 'none',
                }}
              >
              ...
              <div style={hint}>{hold > 0 ? 'keep holding…' : 'press and hold'}</div>
            </div>
          )}
...
const pill: CSSProperties = {
  ...
  color: 'var(--sk-text)',
  cursor: 'pointer',
  transition: 'transform var(--sk-dur-fast) var(--sk-ease), box-shadow var(--sk-dur-fast)',
}
