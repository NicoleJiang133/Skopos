...
export function Play() {
  ...
  const hasMug = objects.some((object) =>
    object.label.toLowerCase().includes("mug"),
  );
  ...
  useEffect(() => {
    const onKeyDown = (event: KeyboardEvent) => {
      if (phase !== "prompt" || event.target instanceof HTMLInputElement)
        return;
      const choice = Number(event.key);
      if (!currentPrompt || choice < 1 || choice > currentPrompt.choices.length)
        return;
    ...
    <div style={{ position: "relative", height: "100%", overflow: "hidden" }}>
      ...
      {!isSummary && currentObject && phase === "walking" && (
        <>
          ...
          <div style={walkingCaption}>
            approaching the {currentObject.label}…
          </div>
        </>
      )}
      ...
      {!isSummary && (
        <div className="sk-panel" style={hud}>
          ...
          <div style={taskLine}>
            Task:{" "}
            {hasMug ? "fetch the mug from the table" : "explore your venue"}
          </div>
          <div
            style={progress}
