...
const DEMO_OBJECTS: SceneObjectLite[] = [
  ...
  { id: 'r1', label: 'rug', material: 'wool', hazards: ['trip', 'low_contrast'] },
  { id: 'c1', label: 'cable', material: 'rubber', hazards: ['trip'] },
  { id: 'd1', label: 'glass door', material: 'glass', hazards: ['reflective'] },
  { id: 'p1', label: 'plant pot', material: 'terracotta', hazards: ['fragile'] },
...
export function Play() {
  ...
  const choose = useCallback(
    (choice: ActionChoice) => {
      ...
      setCounts((previous) => ({ ...previous, [choice.strategy]: previous[choice.strategy] + 1 }))
    ...
    <div style={{ position: 'relative', height: '100%', overflow: 'hidden' }}>
      ...
      {!isSummary && (
        <div className="sk-panel" style={hud}>
          ...
          <div style={progress}>3 / 9 objects</div>
        </div>
      )}
      ...
      <div style={bars}>
        {STRATEGIES.map((strategy) => (
          <div key={strategy.id} style={barRow}>
            ...
            <div style={barTrack}>
              <div style={{ ...barFill, width: `${(counts[strategy.id] / max) * 100}%` }} />
            </div>
          </div>
        ))}
      </div>
...
function hotspotPosition(index: number, total: number) {
  return { position: 'absolute' as const, ...tagPosition(index, total), zIndex: 2 }
}
